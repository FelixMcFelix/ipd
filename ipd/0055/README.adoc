...
The linked commit does not wire up upstream clients such as `ip` to fill out this information; we use this in the https://github.com/oxidecomputer/opte:[OPTE kernel module] after packet encapsulation and decapsulation, which benefits both viona and the downstream NICs used for packet transmit on the order of several hundred Mbps.
