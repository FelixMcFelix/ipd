...
These capabilities are extensions to the existing `MAC_CAPAB_HCKSUM` and `MAC_CAPAB_LSO` queries which can be handled by a provider's `mc_getcapab`.
...
There is ample space here for expansion in future, e.g., (NV)GRE (<<rfc2784>>/<<rfc7637>>).
...
`ct_encap_max` constrains the largest size which an encapsulation header can be to support this class of offload, possibly as imposed by a field size in a TX request.
...
One constraint in which it should not be overzealous is that outer UDP checksums are allowed to be omitted for tunnelled packets <<rfc6935>><<rfc6936>> – these should only be filled if requested, and omission should be generally permitted in the outer layer.
